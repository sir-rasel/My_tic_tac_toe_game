...
void print_coat();
void input();
void other_player_input();
void checked_update_user();
void checked_update_other_user();
int B_element_checked();
int win_checked();
int lost_checked();
void other_user_gaming_element();
void user_gaming_element();
...
void other_player_input(){ /// Randomly A array update kormu
    printf("\nPlease give other turn: ");
    scanf("%d %d",&n,&m);
    if(n>2 || m>2 || position[n][m]!='*'){
        printf("Invalid, try again between (0-2)\n");
        input();
...
void checked_update_other_user(){ /// Input niya position array B update korlam by randomly
    if(n==0){
        if(m==0) coat[3][3]='O';
